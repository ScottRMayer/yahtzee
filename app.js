...
(function () {
  ...
  function removePlayer(index) {
    ...
    if (played && !window.confirm('Remove ' + player.name + ' and their scores?')) return;
    state.players.splice(index, 1);
    commit();
  }

  function newGame() {
    if (anyScores() && !window.confirm('Start a new game? Scores will be cleared — player names are kept.')) return;
